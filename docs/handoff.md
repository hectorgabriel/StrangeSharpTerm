...
| **M4** (part) | `InventoryViewModel`, `WorkspaceViewModel`, the Lucide icon set, and the window: sidebar, tabs, and a terminal in a pane |

Around 259 tests, all green on both operating systems.

## In flight

Check `gh pr list` first — a pull request may have landed since this was
written. At the time of writing: **host detail and `IDialogService`** (PR #15).

## What to do next, in order

1. **Theme system.** `ThemePalette` in the Swift app carries surfaces, borders,
   accents and swatches; only its terminal colours are ported so far, so the
   window is Fluent's default while terminals are themed. These become
   `DynamicResource`s, and the `.id(themeID)` redraw hack has no counterpart
   here — Avalonia re-evaluates them on its own.
2. **Host and folder editors**, through `IDialogService`. The drafts
   (`HostDraft`, `FolderDraft`) and their validation are in the Swift `App/`.
3. **Splits in the window.** `WorkspaceViewModel` already models panes, axes and
   a focused pane; the window shows one pane at a time.
4. **Menu bar and command palette**, with the two Windows questions the plan
...
- **`STRANGESHARPTERM_INVENTORY`** points the app at a throwaway inventory, which
  is how the window gets tested without touching a real one.
