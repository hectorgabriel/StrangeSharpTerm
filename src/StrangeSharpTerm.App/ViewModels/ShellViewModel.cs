...
public sealed partial class ShellViewModel : ObservableObject
{
    ...
    public ShellViewModel(InventoryViewModel inventory, Func<Connection, TerminalSession>? connect = null)
    {
        Inventory = inventory;
    ...
    public async Task OpenTerminal(Connection connection, SplitAxis? splitting = null)
    {
        ...
        try
        {
            ...
            var view = new TerminalPaneView(session, TerminalPalette.ByName(
                Inventory.Tree.Resolve(connection.Id).Settings.TerminalTheme), _terminals);

            view.TitleChanged += (_, title) => Dispatcher.UIThread.Post(() => Workspace.UpdateTitle(pane.Id, title));
            view.Ended += (_, _) => Dispatcher.UIThread.Post(() => Workspace.PaneExited(pane.Id, null));
        ...
        {
            // A failed connection says why, where the user was looking.
    ...
    /// <summary>Puts the focused pane's view on screen.</summary>
    private void Show()
    {
        RefreshTabs();
