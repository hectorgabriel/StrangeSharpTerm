...
public static class AssistPrompts
{
    ...
    public static string Planner(IReadOnlyList<string> hosts) => string.Join("\n",
        ...
        """{"phases": [{"name": "…", "hosts": ["…"], "task": "…", "capture": "…"}]}""",
        "",
        "Each phase names its hosts and says, in words, what an assistant on each of them",
        "should do. Phases run in order; the hosts within one run together.",
        "",
        ...
        "Later phases refer to it as {{name}}. Only a phase with exactly one host may capture,",
        "because two hosts would produce two values and there would be no single one to carry.",
        "",
